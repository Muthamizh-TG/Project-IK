...
# Colorama for colored terminal output (works on Windows/Linux/Mac)
...
from alpaca.trading.requests import MarketOrderRequest, LimitOrderRequest, OrderSide, TimeInForce, OrderType, GetOrdersRequest
...
SIMULATE_FILLS = False  # Set to False to see real orders in Alpaca (but they won't fill!)
...
# -----------------------
# TIMEOUT BUY SETTINGS
# If no buy signal for X minutes, buy anyway at current price
# -----------------------
ENABLE_TIMEOUT_BUY = True     # Enable timeout buy feature
TIMEOUT_BUY_MINUTES = 5       # Wait 5 minutes before timeout buy
TIMEOUT_BUY_TARGET_PCT = 0.002  # 0.2% net profit target (excluding fees/slippage)
...
class AlpacaInterface:
    ...
    def place_limit_buy(self, symbol: str, qty: float, limit_price: float, tif: str = "gtc"):
        # Use alpaca_trade_api.submit_order() - same as manual_buy.py (works well!)
        # Convert XRP/USD to XRPUSD format for trading
        ...
        order = self.data_api.submit_order(**order_params)
        logging.info(f"Submitted limit buy via alpaca_trade_api: {order.id} {order.symbol} {order.qty} @ ${limit_price}")
        return order
    ...
    def place_market_buy(self, symbol: str, qty: float):
        ...
        order = self.data_api.submit_order(**order_params)
        logging.info(f"Submitted market buy via alpaca_trade_api: {order.id} {order.symbol} {order.qty} @ {order.type}")
        return order
    ...
    def place_market_sell(self, symbol: str, qty: float):
        ...
        order = self.data_api.submit_order(**order_params)
        logging.info(f"Submitted market sell via alpaca_trade_api: {order.id} {order.symbol} {order.qty} @ {order.type}")
        return order

    def cancel_order(self, order_id: str):
        # Use alpaca_trade_api for consistency
        return self.data_api.cancel_order(order_id)
...
class Mover:
    def __init__(self, alpaca: AlpacaInterface, symbol: str, profit_target: float = TARGET_USD_PER_TRADE, 
        ...
        # Timeout buy tracking
        self.last_buy_signal_time = time.time()  # Track when we last had a buy signal
        self.timeout_buy_active = False  # Flag if current position is from timeout buy
    ...
    def evaluate_signals_and_act(self):
        ...
        # Check if we should do a timeout buy (if no signal for 5 minutes)
        time_since_last_signal = time.time() - self.last_buy_signal_time
        timeout_buy_triggered = (ENABLE_TIMEOUT_BUY and 
                                time_since_last_signal >= (TIMEOUT_BUY_MINUTES * 60) and 
                                self.position_qty == 0 and 
                                not self.pending_fill)
        ...
        if self.position_qty == 0:
            ...
            # Show timeout buy status if enabled
            if ENABLE_TIMEOUT_BUY and not can_long:
                minutes_waited = time_since_last_signal / 60
                if timeout_buy_triggered:
                    buy_signal = f"TIMEOUT BUY! (waited {minutes_waited:.1f} min)"
                else:
                    time_remaining = TIMEOUT_BUY_MINUTES - minutes_waited
                    buy_signal += f" (timeout buy in {time_remaining:.1f} min)"
        ...
        if self.position_qty == 0 and can_long and not self.pending_fill:
            # Normal buy signal - reset timeout tracking
            self.last_buy_signal_time = time.time()
            self.timeout_buy_active = False
            self.try_enter_long(current_close, current_atr)
        elif self.position_qty == 0 and timeout_buy_triggered:
            # Timeout buy - no signal for 5 minutes, buy anyway
            print(f"{Fore.RED}[TIMEOUT] TIMEOUT BUY TRIGGERED: No buy signal for {TIMEOUT_BUY_MINUTES} minutes - buying at current price{Style.RESET_ALL}")
            print(f"{Fore.RED}TIMEOUT BUY TRIGGERED: No buy signal for {TIMEOUT_BUY_MINUTES} minutes - buying at current price{Style.RESET_ALL}")
            logging.info(f"TIMEOUT BUY TRIGGERED: No buy signal for {TIMEOUT_BUY_MINUTES} minutes - buying at current price")
            self.timeout_buy_active = True
            self.last_buy_signal_time = time.time()  # Reset timer
            self.try_enter_long(current_close, current_atr, timeout_buy=True)
        else:
            ...
            if self.position_qty > 0:
                ...
                # Determine profit target based on entry type
                if self.timeout_buy_active:
                    # Timeout buy uses lower profit target (0.2% net = 0.6% gross with fees)
                    profit_target = TIMEOUT_BUY_TARGET_PCT + self.fees_slippage
                    target_label = f"timeout target ({TIMEOUT_BUY_TARGET_PCT*100:.1f}% net)"
                    target_price = self.position_entry_price * (1 + profit_target)
                else:
                    # Normal buy uses standard profit target
                    profit_target = self.gross_pct
                    target_label = f"target ({self.gross_pct*100:.1f}%)"
                    target_price = self.position_entry_price * (1 + profit_target)
                
                # Display position status
                logging.info(f"POSITION: Entry=${self.position_entry_price:.4f} | Current=${current_close:.4f} | P&L=${current_pnl:.2f} ({current_profit_pct*100:+.2f}%) | Stop=${self.current_stop_price:.4f} | Target=${target_price:.4f} ({target_label})")
                ...
                if self.timeout_buy_active:
                    trailing_threshold_pct = (TIMEOUT_BUY_TARGET_PCT + self.fees_slippage) * 0.5
                else:
                ...
                # Check stop-loss (use trailing stop if set)
                    ...
                    print(f"{Fore.RED}[EXIT] {stop_type} hit at {current_close:.4f} <= {self.current_stop_price:.4f}: exit{Style.RESET_ALL}")
                    logging.info(f"{stop_type} hit at {current_close:.4f} <= {self.current_stop_price:.4f}: exit")
                    self.force_exit_market(current_close)
                # Check full take-profit target (use appropriate target based on entry type)
                elif self.timeout_buy_active and current_profit_pct >= (TIMEOUT_BUY_TARGET_PCT + self.fees_slippage):
                    print(f"{Fore.RED}[EXIT] Timeout buy target reached ({current_profit_pct:.2%}): exit{Style.RESET_ALL}")
                    logging.info(f"Timeout buy target reached ({current_profit_pct:.2%}): exit")
                    self.force_exit_market(current_close)
                elif not self.timeout_buy_active and current_profit_pct >= self.gross_pct:
                    print(f"{Fore.RED}[EXIT] Full target reached ({current_profit_pct:.2%}): exit{Style.RESET_ALL}")
    ...
    def try_enter_long(self, current_price: float, current_atr: float, timeout_buy: bool = False):
        """
        ...
        Args:
            ...
            timeout_buy: True if this is a timeout buy (uses lower profit target)
        """
        # Check if we already have an open order (prevent duplicates)
        try:
            # Use alpaca_trade_api to check open orders
            trading_symbol = self.symbol.replace('/', '')  # Convert XRP/USD to XRPUSD
            open_orders = self.alpaca.data_api.list_orders(status='open', symbols=trading_symbol)
            if open_orders:
                logging.info(f"Skipping buy - already have {len(open_orders)} open order(s)")
                return
        except Exception as e:
            logging.debug(f"Error checking open orders: {e}")
        
        # Store timeout buy flag for exit logic
        self.timeout_buy_active = timeout_buy
        ...
        if use_market_order:
            ...
            print(f"{Fore.RED}[BUY] Placing MARKET buy: qty={qty} USDsize={usd_size:.2f} at ~${current_price:.4f}{Style.RESET_ALL}")
            logging.info(f"Placing MARKET buy: qty={qty} USDsize={usd_size:.2f} at ~${current_price:.4f}")
            
            # In paper mode with simulate fills, don't actually place order
            if PAPER and SIMULATE_FILLS:
                print(f"{Fore.RED}[FILL] SIMULATED FILL: Paper trading mode - simulating instant fill at ${current_price:.4f}{Style.RESET_ALL}")
                logging.info(f"SIMULATED FILL: Paper trading mode - simulating instant fill at ${current_price:.4f}")
                self.on_enter(qty, current_price)
                return
            ...
            print(f"{Fore.RED}[BUY] Placing LIMIT buy: qty={qty} USDsize={usd_size:.2f} limit={limit_price:.4f}{Style.RESET_ALL}")
        ...
        if self.use_websocket:
            ...
            if use_market_order:
                # Market orders - paper trading can be slow (up to 30 seconds)
                time_waited = 0
                max_wait = 30  # Wait up to 30 seconds for paper trading market orders
                logging.info(f"Waiting for market order fill (paper trading can take up to {max_wait}s)...")
                while time_waited < max_wait:
                    time.sleep(1.0)
                    time_waited += 1
                    # Get order status using alpaca_trade_api
                    try:
                        ord_status = self.alpaca.data_api.get_order(order.id)
                    except Exception:
                        ord_status = None
                    
                    if ord_status and ord_status.filled_qty and float(ord_status.filled_qty) > 0:
                        filled_qty = float(ord_status.filled_qty)
                        avg_fill_price = float(ord_status.filled_avg_price) if ord_status.filled_avg_price else current_price
                        self.on_enter(filled_qty, avg_fill_price)
                        return
                    
                    # Log status every 5 seconds
                    if time_waited % 5 == 0 and ord_status:
                        logging.info(f"Order status after {time_waited}s: {ord_status.status}")
                
                # Market order should have filled by now
                logging.warning(f"Market order not filled after {max_wait}s - checking final status")
                try:
                    ord_status = self.alpaca.data_api.get_order(order.id)
                    if ord_status and ord_status.filled_qty and float(ord_status.filled_qty) > 0:
                        filled_qty = float(ord_status.filled_qty)
                        ...
                        logging.error(f"Market order failed to fill after {max_wait}s: status={ord_status.status if ord_status else 'unknown'}")
                        # Cancel the unfilled order
                        try:
                            self.alpaca.cancel_order(order.id)
                            logging.info(f"Canceled unfilled market order {order.id}")
                        except Exception as cancel_exc:
                            logging.debug(f"Cancel failed: {cancel_exc}")
                except Exception as exc:
                    logging.error(f"Failed to check order status: {exc}")
            else:
                # Limit order - wait longer
                time_waited = 0
                max_wait = 10  # Wait up to 10 seconds for limit order
                while time_waited < max_wait:
                    time.sleep(1.0)
                    time_waited += 1
                    # Get order status using alpaca_trade_api
                ...
                # If not filled, cancel limit order
                logging.info(f"Limit order not filled after {max_wait}s, canceling")
                try:
                    self.alpaca.cancel_order(order.id)
                except Exception as exc:
                    logging.debug(f"Cancel failed: {exc}")
    ...
    def on_enter(self, qty: float, avg_price: float):
        ...
        print(f"{Fore.RED}[ENTERED] Position qty={qty:.6f} entry={avg_price:.4f} (slippage={slippage_bps:.1f}bps){Style.RESET_ALL}")
    ...
    def force_exit_market(self, current_price: float = None):
        ...
        # In paper mode with simulate fills, skip actual order placement
        if not (PAPER and SIMULATE_FILLS):
            try:
            ...
            logging.info(f"SIMULATED SELL: Paper trading mode - simulating instant sell")
        ...
        print(f"{Fore.RED}[{result_emoji.upper()}] Exited position at {exit_price:.4f}, P&L=${pnl:.2f} ({pnl_pct:+.2f}%){Style.RESET_ALL}")
        ...
        self.partial_exit_taken = False
        self.breakeven_moved = False
        self.timeout_buy_active = False  # Reset timeout buy flag
        self.breakeven_moved = False
